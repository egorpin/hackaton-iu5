...
// --- ДАННЫЕ О ПЛАНЕТАХ (без изменений) ---
const planetData = [
...
function calculateOrbitPoints(semiMajorAxis, eccentricity, inclination, longitudeOfAscNode, argOfPeriapsis, pointsCount = 360) {
  ...
  // Улучшенная логика для эллиптических и гиперболических орбит
  const isHyperbolic = e > 1;
  const pointLimit = isHyperbolic ? pointsCount / 2 : pointsCount;

  for (let i = 0; i <= pointLimit; i++) {
    // Для гиперболы рисуем только видимую часть траектории
    const angle = isHyperbolic ? (i / pointLimit) * 1.5 - 0.75 : (i / pointsCount) * 2 * Math.PI;

    let r = (a * (1 - e * e)) / (1 + e * Math.cos(angle));

    // Для гипербол (a < 0) радиус должен быть положительным
    if (isHyperbolic && a < 0) {
      r = Math.abs(r);
    }

    if (r > 0 && r < 100) { // Ограничиваем радиус для стабильности
      const vec = new THREE.Vector3(r * Math.cos(angle), r * Math.sin(angle), 0);
      points.push(vec);
    }
  }

  // Применяем матрицы поворота ко всем точкам
  ...
  return points.map(p => p.applyMatrix4(transformMatrix));
}
...
const CelestialOrbit = ({ elements, color = "#FFD700", opacity = 0.4 }) => {
    ...
    const points = useMemo(() => calculateOrbitPoints(
        elements.semimajor_axis,       // ИСПРАВЛЕНО
        elements.eccentricity,
        elements.inclination,
        elements.ra_of_node,           // ИСПРАВЛЕНО
        elements.arg_of_pericenter     // ИСПРАВЛЕНО
    ), [elements]);
...
const Comet = ({ orbitParams }) => {
    ...
    const animationParams = useMemo(() => {
        if (!orbitParams || orbitParams.eccentricity >= 1) return null; // Не анимируем параболы/гиперболы

        const a = orbitParams.semimajor_axis; // ИСПРАВЛЕНО
        ...
        const omegaRad = THREE.MathUtils.degToRad(orbitParams.ra_of_node); // ИСПРАВЛЕНО
        const wRad = THREE.MathUtils.degToRad(orbitParams.arg_of_pericenter); // ИСПРАВЛЕНО
    ...
    // Статичное положение для не-анимируемых орбит
    if (!animationParams && cometRef.current) {
         cometRef.current.position.set(1, 0, 0); // Просто для примера
    }
...
// Компонент для Планет (без изменений)
...
export default function CometOrbitScene({ orbitParams }) {
  const hasCometData = orbitParams &&
    orbitParams.semimajor_axis &&        // ИСПРАВЛЕНО
